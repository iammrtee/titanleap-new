...
export default function Pricing({ onAudit }) {
  ...
  const handleCheckout = async (planKey, billing) => {
    ...
    <section className="sec pricing" id="pricing">
      <div className="wrap">
              ...
              <div key={i} className="p-card" style={{padding:'24px 26px'}}>
                <div style={{display:'flex',alignItems:'flex-start',gap:'16px'}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <button
                      onClick={() => handleCheckout(s.key, 'once')}
                      disabled={checkingOut === s.key}
                      style={{fontSize:'12px',fontWeight:'700',color:'var(--white)',background:'none',border:'none',borderBottom:'1px solid rgba(255,255,255,.2)',paddingBottom:'2px',transition:'border-color .18s',fontFamily:"'Archivo',sans-serif",cursor:checkingOut===s.key?'wait':'pointer',padding:'0 0 2px'}}
                    >
                      {checkingOut === s.key ? 'Redirecting…' : s.cta}
                    </button>
                  </div>
                </div>
              </div>
          ...
          {plans.map((p, i) => (
            <div key={i} className={`p-card reveal${p.hot?' hot':''}`} style={i>0?{animationDelay:`${i*0.1}s`}:{}}>
              ...
              <button
                ...
                onClick={() => handleCheckout(p.key, annual ? 'annual' : 'monthly')}
                disabled={checkingOut === p.key}
                style={{width:'100%',cursor:checkingOut===p.key?'wait':'pointer',opacity:checkingOut&&checkingOut!==p.key?0.5:1}}
              >
                {checkingOut === p.key ? 'Redirecting to Paystack…' : `Subscribe — $${annual ? p.annual : p.monthly}/mo →`}
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="price-addons">
          ...
          {addonTotal > 0 && (
            <div style={{marginTop:'20px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'12px',background:'rgba(107,33,232,.08)',border:'1px solid rgba(107,33,232,.2)',borderRadius:'12px',padding:'16px 22px',animation:'fadeIn .25s ease'}}>
              <div>
